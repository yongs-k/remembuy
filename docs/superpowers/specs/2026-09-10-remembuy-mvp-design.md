...
8. **컬렉션** `/collection` — 상단에 9개 장소 아이콘, 각 아이콘 테두리에 완성도(%)만큼 채워지는 원형 게이지(도넛형 프로그레스 링, 장소 고유 색상 사용) + `%` 텍스트. 장소 클릭 시 해당 장소의 카테고리별 진행률(바)과, 각 카테고리를 펼치면 보유 품목(체크)/미보유 품목(빈칸+"기록하기" 바로가기)을 도감처럼 표시
